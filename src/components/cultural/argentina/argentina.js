...
class Cultural extends Component {
  ...
  getInfo = async () => {
    ...
    await this.state.data.forEach(element => {
      if(element.type=='titulo' && element.country=='ar'){
        auxindex.push(element.name)
      } 
    });
    await this.state.data.forEach((element) => {
      if(element.country=='ar'){
  ...
  renderList = () => {
    ...
    let srcHeader = Global.url+"get-image-cultural/57";
    let srcFooter = Global.url+"get-image-cultural/58";
    return (
