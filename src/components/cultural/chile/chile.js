...
class Cultural extends Component {
  ...
  getInfo = async () => {
    ...
    await this.state.data.forEach(element => {
      if(element.type=='titulo' && element.country=='ch'){
        auxindex.push(element.name)
      } 
    });
    await this.state.data.forEach((element) => {
      if(element.country=='ch'){
  ...
  renderList = () => {
    ...
    let srcHeader = Global.url+"get-image-cultural/210";
    let srcFooter = Global.url+"get-image-cultural/211";
    return (
