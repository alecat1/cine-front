...
class Material extends Component {
  ...
  async componentDidMount() {
    ...
    async function onPingCall(){
      ...
      try {
        const res = await axios.post(Global.url+"home", { headers: { 'access-token':  token, 'key-token': 'codekey:!m@!K3CjDz*NdInr7Q773u3hj*G%5Kl$zZ^%Uz%Z5v#jf2humR'}});
        auxload = false;
      } catch (err) {
        if(language=='en'){
    ...
    let categoryList = uniqueCategory.map((category,i) => {
      return (
          <div className="checkbox-container-material">
            ...
            <div title={category} className="input-title-mt">{category}</div>
          </div>
      );
    });
    return (     
            ...
            <div className="content-mat" >
              <Row>
                ...
                <Col lx={5} xl={5} className="margen-botones">
                  <div className="block-cb-material"><Form>{categoryList}</Form></div>
                </Col>
              </Row>
            </div>
          </Col>
