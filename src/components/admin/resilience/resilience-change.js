...
import "./resilience-change.css";
import Global from "../../../global";
var jwtDecode = require("jwt-decode");
const cookies = new Cookies();
import Page from "react-page-loading";
import swal from "sweetalert";
class resilienceChange extends Component {
  state = {
    selectedFile: null,
  ...
  async componentDidMount() {
    ...
    await axios.post(Global.url + "rs").then(async (res) => {
      let aux = [];
  ...
  onClickUpload = (e) => {
    ...
    contenido.innerHTML = "<strong>Choose the role that will be able to see this content.</strong>";
    ...
    swal({
      ...
      switch (value) {
        case "agent":
          axios.post(Global.url + "upload-pdf-rs/1", data).then((res) => {
            if (res.status == 200) {
        ...
        case "staff":
          axios.post(Global.url + "upload-pdf-rs/2", data).then((res) => {
            if (res.status == 200) {
              swal("Good job!", "The file " + this.state.nameSelectedFile + " has been updated succesfully!", "success").then(function () {
                location.reload();
              });
      ...
      return (
        <div className="container-admin">
          ...
          <Row className="ml-2 mr-2">
            ...
            <Col lg={6} xl={6}>
              <p className="description-p">Select the material you want to delete on the resilience page.</p>
...
export default resilienceChange;
